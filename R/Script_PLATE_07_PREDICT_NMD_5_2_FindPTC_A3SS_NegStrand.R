#' @title Find Premature Terminal Codon (PTC) for Alternative 3' Spice Site (A3SS) Located on the Negative Strand of the Transcript
#'
#' @description
#' \code{FindPTC.A3SS.NegStrand} finds PTC(s) introduced by alternative exons into protein-coding transcripts.
#'
#' @details
...
#'
#' # Run example
#' results <- FindPTC.A3SS.NegStrand(MarvelObject=marvel,
#'                                 tran_id=tran_id,
#'                                  gene_id=gene_id
#'                                 )
#'
...
FindPTC.A3SS.NegStrand <- function(MarvelObject, tran_id, gene_id) {
    ...
    # 5' cons'
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][1]
    cons.exon.5.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
    cons.exon.5.end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
    ...
    cons.exon.3.end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])

    # alt exon
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][2]
    . <- strsplit(., split="|", fixed=TRUE)[[1]][2]
    alt.exon.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1])

    alt.exon.end <- cons.exon.3.start + 1
    ...
    for(i in 1:length(transcript_ids)) {
        ...
        DNAString <- reverseComplement(DNAString) # Only for -ve strand!
