#' @title Find Premature Terminal Codon (PTC) for Alternative 5' Spice Site (A5SS) Located on the Negative Strand of the Transcript
#'
#' @description
#' \code{FindPTC.A5SS.NegStrand} finds PTC(s) introduced by alternative exons into protein-coding transcripts.
#'
#' @details
...
#'
#' # Run example
#' results <- FindPTC.A5SS.NegStrand(MarvelObject=marvel,
#'                                  tran_id=tran_id,
#'                                  gene_id=gene_id
#'                                  )
#'
...
FindPTC.A5SS.NegStrand <- function(MarvelObject, tran_id, gene_id) {
    ...
    # 3' cons'
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][2]
    cons.exon.3.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
    ...
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][1]
    . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
    alt.exon.end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
