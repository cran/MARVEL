#' @title Find Premature Terminal Codon (PTC) for Alternative 3' Spice Site (A3SS) Located on the Positive Strand of the Transcript
#'
#' @description
#' \code{FindPTC.A3SS.PosStrand} finds PTC(s) introduced by alternative exons into protein-coding transcripts.
#'
#' @details
...
#'
#' # Run example
#' results <- FindPTC.A3SS.PosStrand(MarvelObject=marvel,
#'                                  tran_id=tran_id,
#'                                  gene_id=gene_id
#'                                  )
#'
...
FindPTC.A3SS.PosStrand <- function(MarvelObject, tran_id, gene_id) {
    ...
    cons.exon.3.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1])
    cons.exon.3.end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])

    # alt exon
    . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][2]
    . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
    alt.exon.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
